...
export async function backtest(db, proposedRule, ourPayers = [], opts = {}) {
  ...
  const { rows: blocked } = await db.query(`
    ...
    from payments
    where ${pred.sql} and ($2::text[] <> '{}' and lower(payer) = any($2))
  `, [...pred.params, ours]);
  ...
  const { rows: vendorStats } = await db.query(`
    select lower(recipient) as vendor,
           count(distinct lower(payer)) filter (
             where $2::text[] = '{}' or not (lower(payer) = any($2))
  ...
  return {
    ...
    blast_radius: {
      ...
      independent_payers: vendorStats.reduce((s, v) => s + v.independent_payers, 0),
